...
export class ContactListComponent implements OnInit {
  ...
  filterByChar(char: string): ContactModel[] {
    //splits contactList to parts by fisrt char of contact name string
    let contactListFiltredByChar = this.contactList.filter((contact) => {
      return contact.name.charAt(0).toUpperCase() == char;
    });
    return contactListFiltredByChar; //returning part of array with matching first letter of the name
  }
  ...
  fixLongName(contactName: string): string {
